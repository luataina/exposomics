...
data$compounds <- rep(colnames(conc), each = 11)
...
data$values <- c(apply(compound1, 2, mean), apply(compound2, 2, mean), apply(compound3, 2, mean), apply(compound4, 2, mean), 
                 apply(compound5, 2, mean), apply(compound6, 2, mean), apply(compound7, 2, mean), apply(compound8, 2, mean), 
                 apply(controle2, 2, mean))
...
data$sd <- c(apply(compound1, 2, sd), apply(compound2, 2, sd), apply(compound3, 2, sd), 
             apply(compound4, 2, sd), apply(compound5, 2, sd), apply(compound6, 2, sd), 
             apply(compound7, 2, sd), apply(compound8, 2, sd), apply(controle2, 2, sd))
...
data$sem <- c(apply(compound1, 2, sem), apply(compound2, 2, sem), apply(compound3, 2, sem),
             apply(compound4, 2, sem), apply(compound5, 2, sem), apply(compound6, 2, sem),
             apply(compound7, 2, sem), apply(compound8, 2, sem), apply(controle2, 2, sem))
...
p <- ggplot(data, aes(conc, values, fill = compounds, color = compounds)) +
  geom_line(aes(color = compounds), size = 1) + 
  geom_errorbar(aes(ymin = values - sem, ymax = values + sem), width = 0.09, size = 1) +
  geom_hline(yintercept = 20, alpha = 0.5, lty = 2) +
  geom_point(size = 3) +
  theme_classic() +
  labs(x = "log concentrations (M)", y = "RLU (% E2)", title = "Agonist test", fill = "", shape = "", color = "") +
