# --- pipeline --------------------------------------------------------------- #
#Terminal is written in bash. you can run the whole rcode on it.
#just need to change the directory using "cd" (press space and tab to navigate to your folder)
#Once there, you can double check using "pwd".
#Then you're going to run the programe "Rscript" (run the whole script that you have)
#using Rscript code/"name of the rscript" and will automatically run everything
...
setwd("c:/Documents and Settings/LuaReis/Documents/Postdoc/Results/ER/mixture_project/misc/")
# --- load environment ------------------------------------------------------- #
...
plotter <- function(data, compound, pltsize = 9, pointsize = 2.5, sign_ind = 3){
  ...
  ggplot(data, aes(treatment, fold_change)) +
    ...
    labs(x = "Concentration [M]", y = "Fold of control", title = compound) +
